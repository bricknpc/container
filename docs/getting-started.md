...
| `Dirthara\Container\Contract\ContainerConfigurator` | `bind()`, `singleton()`, `scoped()`, `instance()`, `when()`, `extend()`, `afterResolving()`, `lazy()`, `tag()`, `tagByAttribute()` | Registering entries, [contextual bindings](contextual-bindings.md), [decorators](decorators.md), [callbacks](callbacks.md), [lazy services](lazy.md), and [tags](tags.md). |
