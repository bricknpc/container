...
| `Dirthara\Container\Contract\ContainerConfigurator` | `bind()`, `singleton()`, `scoped()`, `instance()`, `when()`, `delegate()`, `extend()`, `afterResolving()`, `lazy()`, `tag()`, `tagByAttribute()` | Registering entries, [contextual bindings](contextual-bindings.md), [delegates](delegates.md), [decorators](decorators.md), [callbacks](callbacks.md), [lazy services](lazy.md), and [tags](tags.md). |
